...
| 层级 | 数据源 | 用途 | 关键边界 |
| --- | --- | --- | --- |
| 主源 | Tushare Pro | 上市/退市历史、未复权日 K、复权因子、财务报表、股本 | 需要 Token 和相应接口权限 |
| 显式备用 | AKShare | 日 K、复权因子、当前证券列表 | 只有用户手动选择才启用；当前列表存在生存者偏差 |
| 可选 | TickFlow | 用户已有套餐时使用其声明支持的数据集 | 不再是隐式兜底 |

任何来源报错、返回空数据或字段校验失败都会留下本地回执。系统不会自动换源，也不会用随机数、演示数据或旧结果伪装本次成功。

```mermaid
flowchart LR
    A["Tushare / 用户明确选择的来源"] --> B["标准化适配器"]
    B --> C["质量门禁与回执"]
...
1. 复制 `.env.example` 为 `.env`，填写所选数据源需要的凭据；
...
Tushare 的接口权限与积分要求可能变化，部署前应核对其[官方接口文档](https://tushare.pro/document/2)。AKShare 来自公开数据整理，结构和可用性可能随上游变化，因此只作为用户主动选择的备用源。

## 已知限制

- 排除政策因素不代表政策风险消失，只代表系统不尝试预测政策；
- 技术和财务因子无法覆盖停牌、重大事项、流动性枯竭等全部风险；
- AKShare 当前证券列表不能还原完整退市历史，不适合直接做无生存者偏差的长期回测；
