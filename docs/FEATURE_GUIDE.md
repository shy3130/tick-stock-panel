...
- 展示 enriched 行情快照与迷你蜡烛图；
- 读取已配置扩展数据的字段；
- 在实时能力可用时刷新行情。实时能力缺失时，行情列会降级而不会借由未授权外部源补齐。
...
交易页提供五个页签：持仓、单笔详情、计划台、账户和桥接规划。桥接规划目前仅是占位说明，**没有**向交易软件发送信号或下单的实现。

单笔交易由事件驱动：建档、准备/修订、成交、加仓、止盈、止损、调整和全部平仓。系统将：

- 把 `trade_events.jsonl` 作为只追加的历史事实流；
- 把 `decision_audit.jsonl` 作为只追加、永不清理的决策审计流；
- 在服务端重算成本、投入金额和已实现盈亏；
- 拒绝已平仓单继续写入；
- 在门禁拦截与确认绕行时都留下审计记录。

账户页维护资金基数、单标的比例上限、期限资金等门禁输入，资金变化只允许追加。组合快照实时派生 NAV、持仓和健康状态。
...
- 数据源与 capability 重探测；
- AI profile（OpenAI 兼容、ACP、Codex CLI）、全局默认、按功能选择和受控 fallback；
...
| 财务 | 支持，受 `financial` capability 约束 | 视本地覆盖 | 不适用 | 不适用 |
...
1. 在“设置 → AI”创建一个可用 profile，选择默认或按功能分配；
2. 先在个股/财务/复盘/Agent 等低风险研究入口验证输出；
