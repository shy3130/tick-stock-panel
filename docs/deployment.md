...
面板使用用户名和密码登录。旧版单用户密码会自动迁移为用户名 `admin` 的管理员账户,原数据复制到 `data/users/admin/`,旧文件保留用于核对。
...
1. 读取 `AUTH_PASSWORD`
2. 创建用户名为 `admin` 的管理员,密码哈希写入 `data/user_data/users.sqlite3`
3. **之后这个环境变量就不再被读取** —— 是一次性的初始化

设完后即可用用户名 `admin` 和这个密码登录。后续改密码请使用页面账户功能。
...
- 明文密码只存在于 `.env` / 环境变量中,SQLite 仅保存 PBKDF2 哈希和会话 token 哈希

不要删除 `users.sqlite3`;它包含全部账户、会话和邀请码状态。修改管理员密码前应先备份整个 `data/` 目录。

### 邀请成员

管理员登录后进入「设置 → 账户管理」生成一次性邀请码。成员在注册页填写邀请码、用户名和密码后,会获得独立的自选、策略、监控、告警、报告与界面偏好。
