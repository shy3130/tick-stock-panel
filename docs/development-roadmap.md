...
| ID | 功能 | 工作量 | 关键风险 | 依赖 |
|---|---|---|---|---|
| **B1** | **Shadow Account**（从盈利 roundtrip 抽 if-then 规则→回放成影子组合→delta-PnL 归因） | XL / 门槛采集中 | Vibe 源 ~3654 行；Trade Journal 已加诊断价值反馈 JSONL，先收集"是否值得回放"证据；两个软接缝（见 CONTEXT.md）：fills 需重新上传解析或加字段、shadow(价格pnl) vs actual(total_pnl含分红)需先定分红口径 | Trade Journal MVP（✅） |
| **B2** | Trade Journal 增强：多账户合并 / 增量导入去重 / Hybrid 叙事（聚合数字，opt-in） | ✅ 已提交(本提交) | 多账户通过 `account_id` 隔离 FIFO；追加导入用归一化 fills/events source 去重；叙事为本地聚合摘要，不送原始流水给 LLM | Trade Journal MVP（✅） |
...
- **B1**（Shadow Account，XL）——**门槛采集中**：Trade Journal 已记录诊断价值反馈，够好再启动；启动时按 CONTEXT 软接缝处理 fills/分红口径。
- **B2**（Trade Journal 增强，✅ 已提交；多账户/追加去重/本地聚合摘要已落地）
