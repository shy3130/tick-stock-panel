...
- 制度边界：历史名称确定当日 ST 标记；symbol/date 确定 `main_10`、`star_20`、`chinext_20` 或 `beijing_30`，创业板以 2020-08-24 切换，科创板/北交所在各自启用日前 fail-closed；仅在基础板块制度有效后，ST 才覆盖为 `st_5`。制度、名称、`ztj` 任一缺失即删失。
