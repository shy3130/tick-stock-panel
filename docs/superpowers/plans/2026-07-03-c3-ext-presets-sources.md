...
- `write_ext_parquet` 的去重键会把有 `symbol` 列的数据按 symbol 去重；新闻一股多篇必须避开 `symbol` 列误去重。
...
**已知坑（设计期已定对策）：**
1. `write_ext_parquet` 的去重键硬编码为"有 symbol 列则按 symbol"（`ext_data.py:408,421`）。**新闻**一股一天多篇文章，若含 `symbol` 列会被去重成 1 行——新闻表**不设 `symbol` 列**，首列放文章唯一键 `uid`（art_code），个股关联用 `code` 列 + `symbol_map={"type":"mapped","col":"股票代码"}` 之外的 code_map route（见任务 6）。其余 5 源一 symbol 一行（或一 symbol 一分区一行），不受影响。
...
3. EM datacenter `pageSize` 上限 500，全市场源必须翻页循环（任务 0 的 `get_json_paged`）。
...
- [ ] **步骤 3：实现（参考 Vibe `agent/backtest/loaders/_http.py:46` 的 HostThrottle 语义：per-bucket 最小间隔+抖动；现文件的全局锁节流升级为按 host 分桶）**
...
- [ ] **步骤 1：改造 `_presets()` 与 `fetch_preset`**
...
- [ ] **步骤 1：抓真实响应落 fixture（先跑接口，字段以实测为准）**
...
- [ ] **步骤 3：运行验证失败**（`_flatten_lockup_rows` 不存在）

- [ ] **步骤 4：实现 preset + flatten + fetcher**
...
- [ ] **步骤 6：Commit** `git commit -am "feat(ext): lockup expiry preset ext_lockup_em (C3-1)"`

---

### 任务 2：股东户数 `ext_holder_count_em`（全市场最新披露，snapshot）
...
- [ ] **步骤 1：fixture** `em_holder_count.json`（同款探测脚本，reportName/columns 换上面的，无 filter）
- [ ] **步骤 2：测试** `test_flatten_holder_rows`：断言键集 `{"symbol","code","end_date","holder_num","holder_num_change","holder_num_ratio","avg_hold_amt","avg_hold_num","total_market_cap"}`
- [ ] **步骤 3：实现**：preset `id="ext_holder_count_em"`, label 股东户数, **mode="snapshot"**（最新披露快照，一 symbol 一行，天然按 symbol 去重覆盖）；flatten 直接字段搬运（END_DATE 截 10 位）；fetcher 用 `get_datacenter_paged(..., max_pages=15)`，无日期 filter。
- [ ] **步骤 4：全链路** `fetch_preset("ext_holder_count_em", ...)` 行数 ≳ 5000
- [ ] **步骤 5：Commit** `git commit -am "feat(ext): shareholder count preset (C3-2)"`
...
- [ ] **步骤 1：fixture** `em_margin.json`（探测 600519）
- [ ] **步骤 2：测试** `test_flatten_margin_rows`：键集 `{"symbol","code","trade_date","financing_balance","financing_buy","financing_repay","short_balance","short_volume","margin_total_balance"}`
- [ ] **步骤 3：实现**：preset `id="ext_margin_em"`, mode="timeseries"；fetcher 遍历 watchlist（读法抄 `app/services/extend_history.py:_load_watchlist_symbols`，即 `data/user_data/watchlist.parquet` 的 symbol 列），仅 A 股后缀（.SH/.SZ/.BJ），逐股 `filter=(SCODE="{六位code}")`、每股 1 页 pageSize=30；聚所有股票行后按 trade_date 分组逐日期 `rows_to_parquet`（分区内一 symbol 一行，去重键成立）。watchlist 为空时抛 `ValueError("自选股为空，两融明细按自选股拉取")`。
- [ ] **步骤 4：全链路 + Commit** `git commit -am "feat(ext): margin trading preset, watchlist-scoped (C3-3)"`

---

### 任务 4：大宗交易 `ext_block_trade_em`（全市场按日，timeseries）
...
- [ ] **步骤 1-3：** fixture `em_block_trade.json` → 测试键集 `{"symbol","code","trade_date","name","close","deal_price","premium_ratio","deal_volume","deal_amount","buyer","seller"}` → 实现（mode="timeseries"，fetcher 拉当天、`snapshot_date=当天`；**注意**同一 symbol 当天可有多笔大宗，分区内按 symbol 去重会丢行——flatten 后按 symbol 聚合：笔数 `deal_count`、金额加总 `deal_amount`、加权均价 `deal_price`、溢价率按金额加权，buyer/seller 取金额最大一笔。测试须覆盖"两笔同 symbol 聚成一行"）
- [ ] **步骤 4：全链路 + Commit** `git commit -am "feat(ext): block trade preset with per-symbol aggregation (C3-4)"`

---

### 任务 5：研报/EPS `ext_research_eps_em`（自选股范围，snapshot）
...
- [ ] **步骤 1：fixture** `em_research.json`（探测 600519，记录完整字段）
- [ ] **步骤 2：测试**：键集（以 fixture 定稿为准，目标 schema）`{"symbol","code","publish_date","title","org","rating","eps_this_year","eps_next_year"}`；EPS 字段缺失时容 None
- [ ] **步骤 3：实现**：preset `id="ext_research_eps_em"`, mode="snapshot"（每股保留最新 N=10 篇聚合行不现实——snapshot 去重键是 symbol，故**每股聚成一行**：最新评级、评级机构数、近90天研报数、EPS 一致预期均值）；fetcher 遍历 watchlist A 股逐股拉取。白名单已含 reportapi（任务 0）。
- [ ] **步骤 4：全链路 + Commit** `git commit -am "feat(ext): research/EPS consensus preset, watchlist-scoped (C3-5)"`

---

### 任务 6：新闻 `ext_news_em`（自选股范围，timeseries，无 symbol 列）
...
- [ ] **步骤 1：fixture** `em_news.json`（探测"贵州茅台"）
- [ ] **步骤 2：测试**：`test_flatten_news_rows` 键集 `{"uid","code","股票代码","publish_date","title","source","url"}`；**断言 rows 无 "symbol" 键**（坑1：防止按 symbol 去重塌行）；`test_news_dedup_key_is_uid`：两篇同股票文章 flatten 后 uid 不同
- [ ] **步骤 3：实现**：preset `id="ext_news_em"`, mode="timeseries"，fields 首列 `uid`（EM art_code 或 url hash），`symbol_map={"type":"mapped","col":"股票代码"}`（映射用中文列，不产生名为 symbol 的列）；fetcher 遍历 watchlist、每股 pageSize=10，按 publish_date 分区写。
- [ ] **步骤 4：全链路验证分区内多篇文章共存（`pl.read_parquet(...).filter(pl.col("股票代码")=="600519") 行数>1` 若当天多篇）+ Commit** `git commit -am "feat(ext): per-stock news preset with uid dedup key (C3-6)"`

---

### 任务 7：收尾回归

- [ ] 全量 `uv run --extra dev pytest -q`；前端扩展数据页逐表点"获取数据"过一遍；6 张表在概念/行业页同款 UI 下可浏览。
- [ ] `docs/vibe-trading-migration-candidates.md` C3 条目标注已完成范围与"北向已剔除"复述。
- [ ] Commit：`git commit -am "docs: mark C3 sources landed"`
