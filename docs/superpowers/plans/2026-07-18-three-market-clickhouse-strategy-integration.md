...
- [ ] 规格中的数据契约、三市场规则、共享策略、UI、异常和回滚均有对应任务。
- [ ] 计划中没有占位描述或未定义接口。
- [ ] `known_at`、市场代码、币种和信号阶段命名在所有任务中一致。
- [ ] 共享策略核心未复制到 TickFlow 源码。
- [ ] 线上部署不建立第二套 Longbridge 订阅。
