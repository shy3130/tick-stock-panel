...
面向单只 A 股的证据优先研究工作区。每次研究独立保存数据截止时间、来源状态、原始安全链接和引用编号，模型只能基于服务端收集并脱敏后的证据包生成结论。

- 覆盖日线、实时缓存、财务与估值、题材/热度/资金流、策略信号、研报、公告和可选联网新闻线索
- 采集失败、能力缺失和空结果会显示为可审阅的证据状态，不会被伪装成正常数据
- 研究结论将事实、推断和待复核事项分开；网页标题和摘要仅作线索，不自动视为已验证事实
- 为保护本机数据源和模型服务，运行数有小型并发上限；达到上限时页面会提示稍后重试

使用方式、接口和安全边界见 [research-agent.md](./research-agent.md)。
