...
```ini
AUTH_PASSWORD=管理员密码       # 初始化用户名 admin;至少 6 位;仅首次生效
INVITE_CODES=CODE1,CODE2      # 每个邀请码可注册一个普通账户
```

现有单用户 `auth.json` 会自动迁移为 `admin` 账户。自选、策略、监控、告警和报告按账户隔离;行情、财务、指数与扩展数据继续共享。
