...
本项目默认使用 Tushare 插件。自定义数据源是一个可选扩展: 外部 HTTP 服务负责取数和整理, 本项目只把返回结果映射成内部标准字段, 然后复用现有存储、指标、enriched、策略和前端展示逻辑。
...
| 分钟K | `minute` | 返回指定资产和周期的分钟线 |
| 财务 | `financial` | 按 `table` 参数返回指标、三大报表或股本历史 |
