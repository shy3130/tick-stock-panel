...
2. **P1 去 A 股硬编码（低-中）**：indicators pipeline 按 asset_type 分流关闭涨跌停段；交易时段表按市场；符号层已就绪。
3. **P2 港股复权口径（中）**：接港股复权因子或明确"未复权 + UI 标注"。
4. **P3 功能适配（中）**：个股分析/回测/监控按市场配置化（基准、费用、prompt）。
5. **不做**：涨停梯队/真假涨停/情绪复盘/A 股参考数据 —— 概念不适用，港股需另设。
