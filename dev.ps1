# tickflow-stock-panel - backend launcher (Windows / PowerShell)
#
# Usage:
#   .\dev.ps1
...
# ===== 2. Port check - kill anything listening on the target port =====
...
Write-Host '|  tickflow-stock-panel (backend)             |' -ForegroundColor Blue
Write-Host '|                                              |' -ForegroundColor Blue
Write-Host "|  backend   http://localhost:$BackendPort"      -ForegroundColor Blue
Write-Host '|                                              |' -ForegroundColor Blue
Write-Host '|  Ctrl-C closes backend                       |' -ForegroundColor Blue
Write-Host '+----------------------------------------------+' -ForegroundColor Blue
Write-Host ''
...
function Cleanup-All {
    ...
    if ($backendChildPid) {
        $null = & cmd /c "taskkill /F /T /PID $backendChildPid 2>nul"
    }
    if ($backendJob) {
...
try {
    ...
    while ($true) {
        ...
        if ($backendJob.State -ne 'Running') {
            Log-Warn 'backend exited; shutting down...'
            break
        }
