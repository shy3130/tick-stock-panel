...
- **AI 短线池**：在 `/agent` 选择「AI短线池」示例即可运行服务端固定的 `short_momentum_quality_v1`；只读取最新可信 canonical 日线，返回 5–12 只带完整条件证据的研究观察标的。候选与排序由确定性筛选产生，AI 只解释、不增删重排；结果可查看个股、批量加自选或显式送策略回测，不自动启动研究/监控，也不构成投资建议。
