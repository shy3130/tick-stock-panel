...
- **AI 短线池**：在 `/agent` 选择「AI短线池」示例即可运行服务端固定的 `short_momentum_quality_v1`；只读取最新可信 canonical 日线，输出上限可设为 5–12 只，无命中时返回空观察池，每只候选均带完整条件证据。候选与排序由确定性筛选产生，AI 只解释、不增删重排；结果可查看个股、批量加自选或显式送策略回测，不自动启动研究/监控，也不构成投资建议。
