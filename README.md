...
仓库名 `tickflow-stock-panel` 为兼容远程地址、发布流程和既有部署保留；当前产品边界以
“量化研究与回测后端”为准，不再按名称恢复面板前端。
...
3. [ARCHITECTURE.md](./ARCHITECTURE.md)：目录、模块边界和新增文件规则。
...
├─ backend/
│  ├─ app/                 # FastAPI 生产后端
│  ├─ research/            # 按领域组织的活跃研究与 legacy 历史入口
│  ├─ scripts/             # 后端运维脚本
│  ├─ tests/               # 自动化测试
│  ├─ pyproject.toml
│  └─ uv.lock
...
完整边界、命名规则与 legacy 边界见 [ARCHITECTURE.md](./ARCHITECTURE.md)。
...
canonical enriched 日线，默认使用结束日全部上市非 ST 股票、调用现有 Matrix 引擎回测，并输出
...
如需研究抽样，可显式传入 `--universe-size 200 --seed 20260723`。默认全量模式不抽样，
相同数据、区间和策略会产生相同的协议哈希与核心结果；运行耗时、
...
已下线面板手册和 UI 截图保存在 `docs/archive/`，不属于当前产品结构。
