...
Sycee 在保留上游行情、选股、回测和监控能力的基础上,将个人研究与服务器协作能力放在独立模块中维护:

| 能力 | 说明 |
| :--- | :--- |
| **研究账本** | 从个股预览一键加入研究,自动记录来源并去重;也可补充假设、证据、失效条件和下一步 |
| **私有访问** | 公开品牌入口、邀请码注册和独立用户账户 |
| **用户隔离** | 自选、策略配置、监控记录、研究账本等私人数据按用户隔离,行情和基础计算共享 |
| **产品工作台** | Sycee 品牌、产品化导航、响应式应用外壳和服务器部署入口 |
| **上游同步** | 以 merge commit 接收原版升级,并通过独立 Sycee 模块降低冲突范围 |

研究账本入口位于 **策略与回测 → 研究账本**。打开任一个股预览后点击 **加入研究**,即可先保存为待整理记录;捕获结果支持立即查看或撤销。专属功能边界、数据位置和上游接入点见 [docs/sycee-features.md](./docs/sycee-features.md)。
