...
| TDX A 股 trans 路由 | 发布 catalog + DuckDB read-only | 按交易日定位逐年 trans 快照（staged，preliminary→final） | `FQUANT_SNAPSHOT_ROOT_CATALOG` + `FQUANT_SNAPSHOT_ROOT_ENGINE_A{,_PRELIMINARY,_TRANS_ARCHIVE}` |
