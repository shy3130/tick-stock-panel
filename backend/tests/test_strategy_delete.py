...
def test_delete_strategy_uses_current_user_runtime_and_storage(monkeypatch, tmp_path):
    alice = UserIdentity(id="alice", username="alice")
    alice_dir = tmp_path / "users" / "alice" / "strategies" / "custom"
    bob_dir = tmp_path / "users" / "bob" / "strategies" / "custom"
    alice_dir.mkdir(parents=True)
    bob_dir.mkdir(parents=True)
    alice_path = alice_dir / "target.py"
    bob_path = bob_dir / "target.py"
    alice_path.write_text(_strategy_code("target"), encoding="utf-8")
    bob_path.write_text(_strategy_code("target"), encoding="utf-8")

    engine = StrategyEngine(strategy_dirs=[alice_dir])
    monitor = _MonitorEngine()
    runtime = SimpleNamespace(strategy_engine=engine, monitor_engine=monitor)
    registry = SimpleNamespace(get=lambda user: runtime)
    repo = SimpleNamespace(store=SimpleNamespace(data_dir=tmp_path))
    app_state = SimpleNamespace(repo=repo, user_runtime_registry=registry)
    request = SimpleNamespace(
        app=SimpleNamespace(state=app_state),
        state=SimpleNamespace(user=alice),
    )
    monkeypatch.setattr(preferences, "get_strategy_monitor_ids", lambda: [])

    with as_user(alice):
        result = delete_strategy("target", request)
