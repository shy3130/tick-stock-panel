...
def request(repo=None):
    return SimpleNamespace(
        app=SimpleNamespace(state=SimpleNamespace(repo=repo or FakeRepo(), quote_service=None))
    )
...
def test_daily_batch_local_fallback_splits_asset_types(monkeypatch):
    ...
    resp = kline.get_daily_batch(
        request(), {"symbols": ["600519.SH", "513050.SH", "02577.HK"], "days": 5}
    )
...
def test_daily_chart_live_bar_stays_separate_from_adjusted_rows(monkeypatch):
    ...
    monkeypatch.setattr("app.services.external_fallback.get_adapter", lambda: adapter)
    monkeypatch.setattr("app.services.external_fallback.adapter._cn_today", lambda: today)
    adjusted_rows = [
        {
            "date": "2026-08-21",
            "open": 1250.0,
    ...
    fields = kline._chart_live_daily_response_fields(
        request(), "600519.SH", adjusted_rows, end=today
    )

    # 未复权 fallback 绝不能被 append 到前复权历史。
    ...
    assert fields["provisional_daily"] == {
        "date": "2026-08-24",
        "open": 10.0,
        "high": 11.0,
        "low": 9.5,
    ...
    assert {key: fields[key] for key in ("degraded", "sources", "fallback_reason")} == {
        "degraded": True,
    ...
    assert (
        kline._chart_live_daily_response_fields(request(), "600519.SH", [], end=date(2026, 8, 21))
        == {}
    )
