...
用法: python run_factor_walkforward.py
"""
...
from factor_dsl import StackVM, formula_to_str, gen_formula
from run_factor_search import build_features, cross_sectional_score
from run_factor_oos import norm_cdf

HERE = Path(__file__).resolve().parent
OUT = HERE.parent / "strategy_factor_walkforward.json"
...
def select_universe():
    ...
    return rng.sample(all_syms, min(N_SYM, len(all_syms)))


def pl_scan():
    import polars as pl
    return pl.scan_parquet(
        str(HERE.parent / "data" / "kline_daily_enriched" / "**" / "*.parquet"),
...
def main():
    ...
    for key, a in sorted(agg.items(), key=lambda kv: (kv[1]["positive_folds"], kv[1]["mean_oos_sharpe"]), reverse=True):
        print(f"  {key:<12} +{a['positive_folds']}/{a['n_folds']} 跑赢null{a['beat_null_folds']}/{a['n_folds']} "
              f"meanOOS={a['mean_oos_sharpe']:+.2f} {'✓稳健' if a['robust_across_regimes'] else '✗不稳健'} | {a['formula']}", flush=True)
