        ...
        for idx, fill in enumerate(sfills):
            ...
            if f.side == "sell" and f.qty > pos + _EPS:
                ...
                if pos <= _EPS:
                    continue
                scale = pos / f.qty
                f = Fill(f.date, f.time, f.symbol, f.name, f.side, pos, f.price, f.amount * scale, f.fee * scale)
            cycle.append(f)
            ...
            if pos <= _EPS and cycle:
                ...
                if nxt is not None and nxt.side == "buy" and nxt.date == f.date:
                    continue
                trips.append(_close_cycle(symbol, cycle, trading_days))
                cycle, pos = [], 0.0
...
def _apply_dividends(trips: list[Roundtrip], div_by_symbol: dict[str, list[CashEvent]]) -> None:
    ...
    for symbol, events in div_by_symbol.items():
        sym_trips = by_symbol.get(symbol, [])
