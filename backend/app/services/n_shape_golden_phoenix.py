...
- 读取边界：只接受 immutable markets generation 的复合 reader（构造注入）；
  禁止 get_enriched_range 合并 overlay 或现有 signal_limit_up 作为替代。
- 能力门禁（优先于一切命中）：复合 reader 缺失时，整份评估返回
  ``unavailable`` + reasons，不降级、不猜口径。
...
class PublishedDailyMarketResearchReader(Protocol):
    """immutable markets generation 的单一复合研究 reader。"""
...
RESEARCH_READER_ATTR = "n_shape_research_reader"
...
def resolve_n_shape_reader(repo: Any) -> PublishedDailyMarketResearchReader | None:
    """从 repository 解析完整复合 reader；缺能力即 None。"""
    reader = getattr(repo, RESEARCH_READER_ATTR, None)
    return (
        reader
        if all(callable(getattr(reader, name, None)) for name in REQUIRED_READER_METHODS)
        else None
    )
...
# ── 涨跌停判定（source exact price） ─────────────────────────────────────


_NON_ST_LIMIT_REGIMES = frozenset({"main_10", "star_20", "chinext_20", "beijing_30"})


def _valid_limit_regime_fact(fact: Any) -> bool:
    if not isinstance(fact, dict):
        return False
    name = fact.get("name")
    is_st = fact.get("is_st")
    regime = fact.get("regime")
    price = fact.get("limit_up_price")
    if not isinstance(name, str) or not name.strip() or not isinstance(is_st, bool):
        return False
    if not isinstance(price, (int, float)) or not math.isfinite(float(price)) or price <= 0:
        return False
    historical_name_is_st = "ST" in name.upper()
    if is_st != historical_name_is_st:
        return False
    if is_st:
        return regime == "st_5"
    return regime in _NON_ST_LIMIT_REGIMES


def _is_limit_up(
    regime_facts: dict[date, dict[str, Any]],
    bar: dict[str, Any],
) -> bool | None:
    """仅以 markets source 同日历史名称与 exact limit price 判定涨停。"""
    raw_close = bar.get("raw_close")
    fact = regime_facts.get(bar["date"])
    if not _valid_limit_regime_fact(fact):
        return None
    source_price = fact["limit_up_price"]
    if raw_close is None or source_price is None:
        return None
    return abs(float(raw_close) - float(source_price)) <= LIMIT_PRICE_TOL
...
def _valid_manifest_sha256(value: Any) -> bool:
    ...
    return isinstance(value, str) and len(value) == 64 and set(value.lower()) <= _SHA256_HEX_DIGITS


def _valid_source_provenance(value: Any) -> bool:
    if not isinstance(value, dict) or set(value) != {"canonical", "markets"}:
        return False
    for source in value.values():
        if not isinstance(source, dict):
            return False
        if not isinstance(source.get("generation"), str) or not source["generation"]:
            return False
        if not _valid_manifest_sha256(source.get("manifest_sha256")):
            return False
    return True
...
def unavailable_envelope(
    *,
    ...
    return {
        ...
        "censored": [],
        "note": (
            "immutable markets generation 复合 reader 是本因子的前置数据能力；"
            "缺失时显式返回 unavailable，不以合并 overlay 或 signal_limit_up 替代"
        ),
    }
...
def _bars_to_dicts(frame: pl.DataFrame, symbol: str) -> tuple[list[dict[str, Any]], dict | None]:
    ...
    for row in rows:
        for field in REQUIRED_RAW_COLUMNS:
            value = row.get(field)
            if value is None or (
                isinstance(value, float) and (value != value or not math.isfinite(value))
            ):
                return [], {
                    "symbol": symbol,
                    "code": "raw_field_missing",
    ...
    *,
    ...
    if off_calendar:
        return [], [
            {
                "symbol": symbol,
                "code": "date_not_in_market_calendar",
                "detail": {"dates": off_calendar[:5]},
            }
        ]

    ordered = sorted(bars_by_date)
    # 逐日涨停标记（None = source 缺少该日期）
    limit_flags: dict[date, bool | None] = {}
    for d in ordered:
        limit_flags[d] = _is_limit_up(regime_facts, bars_by_date[d])
    ...
    for t in ordered:
        ...
        if abs(first_bar["raw_open"] - first_bar["raw_high"]) <= LIMIT_PRICE_TOL:
            censored.append(_censor("one_price_board", event_date=str(t)))
            continue
        ...
        pre20_avg = (
            sum(b["volume"] for b in pre20_bars) / len(pre20_bars)
            if len(pre20_bars) == 20
            else None
        )
        ...
        suspended = [
            str(d)
            for d in calendar[t_pos + 1 : t_pos + POST_WINDOW_MAX + 1]
            if d not in bars_by_date
        ]
        if suspended:
            censored.append(_censor("suspended_in_window", dates=suspended[:5]))
            continue
        post_days = calendar[t_pos + 1 : t_pos + POST_WINDOW_MAX + 1]
        unknown_post = [str(d) for d in post_days if limit_flags.get(d) is None]
        if unknown_post:
            censored.append(
                _censor(
                    "limit_regime_unknown",
                    event_date=str(t),
                    dates=unknown_post[:5],
                )
            )
            continue
        ...
        for offset in range(1, POST_WINDOW_MAX + 1):
            ...
            adjust_avg = (
                sum(x["volume"] for x in adjust_bars) / len(adjust_bars) if adjust_bars else None
            )
            if shrink_done_day is None and adjust_avg is not None:
                if (
                    adjust_avg / ref_vol <= VOLUME_SHRINK_RATIO
                    and adjust_avg / pre20_avg <= VOLUME_PRE20_RATIO
                ):
                    shrink_done_day = d
            ...
            if VARIANT_VOLUME_BREAKOUT not in confirmed and adjust_avg is not None:
                ...
                ma5 = (
                    sum(x["raw_close"] for x in ma5_bars) / len(ma5_bars)
                    if len(ma5_bars) == 5
                    else None
                )
                ma10 = (
                    sum(x["raw_close"] for x in ma10_bars) / len(ma10_bars)
                    if len(ma10_bars) == 10
                    else None
                )
                ...
                if (
                    ...
                    confirmed[VARIANT_VOLUME_BREAKOUT] = _build_event(
                        symbol=symbol,
                        variant=VARIANT_VOLUME_BREAKOUT,
                        event_date=t,
                        confirm_date=d,
                        calendar=calendar,
                        ...
                        ma5=ma5,
                        ma10=ma10,
                        ma_basis="ma5" if ma5_ok else "ma10",
                        adjust_avg=adjust_avg,
                        regime_fact=regime_facts[t],
                        pre20_avg=pre20_avg,
            ...
            if VARIANT_SECOND_LIMIT_UP not in confirmed and limit_flags.get(d) is True:
                confirmed[VARIANT_SECOND_LIMIT_UP] = _build_event(
                    symbol=symbol,
                    variant=VARIANT_SECOND_LIMIT_UP,
                    event_date=t,
                    confirm_date=d,
                    calendar=calendar,
                    price_position=price_position,
...
def _aggregate_population(
    ...
    *,
    ...
    for symbol in sorted(by_symbol):
        ...
        group = sorted(
            by_symbol[symbol],
            key=lambda s: (calendar_index[s["anchor_date"]], str(s.get("variant", ""))),
        )
        for sample in group:
    ...
    result: dict[str, Any] = {
        ...
        "ci95_high": None,
        "post_cost_mean": None,
        "post_cost_ci95_low": None,
        "post_cost_ci95_high": None,
    }
...
def _population_stats(
    samples: list[dict[str, Any]], calendar_index: dict[date, int]
) -> dict[str, Any]:
    ...
    for split in ("is", "oos"):
        ...
        output[split] = {
            ...
            "stats_by_horizon": {
                horizon: _aggregate_population(
                    subset, calendar_index=calendar_index, horizon=horizon
                )
                for horizon in FORWARD_HORIZONS
            },
        }
    ...
    *,
    ...
    populations = {
        ...
        "events_volume_breakout": [
            normalize(item, "confirm_date")
            for item in events
            if item["variant"] == VARIANT_VOLUME_BREAKOUT
        ],
        "events_second_limit_up": [
            normalize(item, "confirm_date")
            for item in events
            if item["variant"] == VARIANT_SECOND_LIMIT_UP
        ],
    }
...
def _build_event(
    *,
    ...
    evidence = [
        _evidence("event_historical_name", regime_fact["name"], "==", "source_fact"),
        _evidence("event_historical_is_st", regime_fact["is_st"], "==", regime_fact["is_st"]),
        _evidence("event_limit_regime", regime_fact["regime"], "==", regime_fact["regime"]),
        _evidence(
            "event_source_limit_up_price",
            regime_fact["limit_up_price"],
            "==",
            "source_fact",
        ),
        ...
        _evidence(
            "adjust_volume_vs_first_board",
            round((adjust_avg or 0) / ref_vol, 4),
            "<=",
            VOLUME_SHRINK_RATIO,
        ),
        _evidence(
            "adjust_volume_vs_pre20",
            round((adjust_avg or 0) / pre20_avg, 4),
            "<=",
            VOLUME_PRE20_RATIO,
        ),
        _evidence(
            ...
            str(confirm_date),
            "within",
            f"[event+{POST_WINDOW_MIN}, event+{POST_WINDOW_MAX}]",
        ),
    ]
    if variant == VARIANT_VOLUME_BREAKOUT:
        ...
        evidence.extend(
            [
                _evidence(
                    "confirm_volume_vs_adjustment",
                    round(bar["volume"] / (adjust_avg or 1), 4),
                    ">=",
...
def evaluate_n_shape(
    *,
    ...
    if not _valid_source_provenance(source_provenance):
        return unavailable_envelope(
            start=start,
            end=end,
            reasons=["pit_source_provenance_invalid"],
        )

    lookup_start = start - timedelta(days=_CALENDAR_WARMUP_DAYS)
    calendar = sorted(reader.market_days(lookup_start, end))
    if len(calendar) < PRIOR_CLEAN_DAYS + 1:
        return unavailable_envelope(start=start, end=end, reasons=["market_calendar_insufficient"])

    if symbols is None:
        symbols = sorted({str(symbol) for symbol in reader.universe(start, end) if str(symbol)})
    else:
    ...
    for symbol in symbols:
        regime_facts = reader.limit_regime_facts(symbol, lookup_start, end)
        frame = reader.daily_bars(symbol, lookup_start, end)
