...
不引入任何外部行情数据源 —— 仅消费本机 enriched parquet。

数据访问: 全程走 ``repo.get_enriched_range`` 这一公开历史入口(repository 内部
负责 scan + warmup + compute_all), 不直接 scan_parquet / 不调用上游的
``compute_indicators(df, needed=...)`` 等带 needed 参数的签名。大范围回填时按交易日
分批调用 ``get_enriched_range``, 控制内存峰值。

性能设计:
- run_regime_batch 用 polars ``group_by("date").agg(...)`` 一次聚合多日, 非逐日循环。
- 分批只扫目标区间; 批间独立, 每批边界指标由 repository 内置 warmup 保证正确。
...
def _compute_subscores(metrics: dict) -> dict:
    ...
    # 抗跌维度: 只用 strong_down_pct(大跌股≤-3%占比), 不用 down_pct — 后者与 profit 的
    # up_pct 是同一信息的正反面, 叠加会放大两极化、挤压震荡区间。
...
def _scan_enriched_fallback(repo, start: date, end: date) -> pl.DataFrame | None:
    """读取 [start, end] 的 enriched 数据(含信号列), 内存受控。

    全程走公开入口 ``repo.get_enriched_range`` (repository 内部负责 scan + warmup
    + compute_all)。仅在 enriched 数据缺失时返回 None。

    内存控制: 范围超过 batch_days 个交易日时按交易日切片, 逐批读取后 concat。
    """
    ...
    if len(target_dates) <= batch_days:
        df = repo.get_enriched_range(target_dates[0], target_dates[-1])
    ...
    logger.info("regime scan: %d 天分 %d 批 (每批≤%d天)",
                len(target_dates), len(batches), batch_days)
    parts: list[pl.DataFrame] = []
    for bs, be in batches:
        df = repo.get_enriched_range(bs, be)
...
def run_regime_batch(repo, start: date, end: date) -> pl.DataFrame:
    ...
    性能: 走 repo.get_enriched_range(分批控制内存), 按 date group_by 聚合,
    不逐日重算。返回完整时序 DataFrame(可能为空)。
    """
    if start > end:
        return pl.DataFrame()
...
def upsert_regime_history(data_dir: Path, new_rows: pl.DataFrame) -> None:
    ...
    schema 兼容: 旧 parquet 可能缺新列, concat 前给旧数据补缺失列(null),
    让旧 parquet 首次重写时自动迁移到新 schema。
    """
...
def regime_t1_entry_mask(
    panel: pl.DataFrame,
    data_dir: Path,
    allowed_states: set[str] | None = None,
    min_score: float | None = None,
) -> pl.Series | None:
    """构建 T-1 环境入场掩码, 与 panel 行对齐。

    契约:
    - 用 panel 交易日序列中紧邻的**前一交易日**(T-1)环境决定 T 日是否允许入场；
      若该日的 regime 记录缺失则 fail-open，绝不沿用更早的陈旧环境。
    - 缺 regime 数据 → 返回 None (fail-open: 调用方不过滤)。
    - 只影响入场, 不改变退出。

    返回:
    - None: 无 regime 数据 / 未配置过滤条件, fail-open。
    - pl.Series[bool]: 与 panel 行对齐, True=允许入场。
    """
    if allowed_states is None and min_score is None:
        return None  # 未配置过滤条件

    regime_df = load_regime_history(data_dir)
    if regime_df.is_empty() or "date" not in regime_df.columns:
        return None  # fail-open: 无 regime 数据
    ...
    panel_dates = panel["date"].to_list()
    trading_dates = sorted(set(panel_dates))
    previous_date = {
        trading_dates[idx]: trading_dates[idx - 1] if idx > 0 else None
        for idx in range(len(trading_dates))
    }

    # 每个 panel 日期只判定一次。前一交易日没有精确 regime 记录时放行，避免把
    # 数日前的陈旧状态误当作 T-1；warmup 区间通常会提供正式首日的前一交易日。
    date_allow: dict[date, bool] = {}
    for panel_date in trading_dates:
        t1_date = previous_date[panel_date]
        regime = regime_by_date.get(t1_date) if t1_date is not None else None
        if regime is None:
            date_allow[panel_date] = True
            continue
        allowed = True
        if allowed_states is not None:
            state = regime.get("state")
            allowed = allowed and isinstance(state, str) and state in allowed_states
        if min_score is not None:
            score = regime.get("score")
            allowed = allowed and score is not None and float(score) >= min_score
        date_allow[panel_date] = allowed

    return pl.Series([date_allow.get(d, True) for d in panel_dates])
