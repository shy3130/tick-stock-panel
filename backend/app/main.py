...
async def lifespan(app: FastAPI):
    ...
    # ⚠️ 必须在能力探测之前加载 —— detect_capabilities 会按 custom 源补充能力
    # (如 tencent minute → KLINE_MINUTE_BATCH)。顺序颠倒时启动探测的 _PROVIDERS
    # 仍为空, app.state.capabilities 缺 custom 补的能力, 分时/分钟K批量接口 403,
    # 直到每小时 _reprobe_capabilities 才修正 (此前重启必复发)。
    try:
