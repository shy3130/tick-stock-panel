...
def _store(request: Request) -> ResearchStore:
    return ResearchStore(request.app.state.repo.store.data_dir)
...
def create_hypothesis(body: HypothesisIn, request: Request):
    if _has_reserved_t_research_tag(body.tags):
        raise HTTPException(status_code=400, detail="做T研究系统标签只能通过显式确认入口创建")
    try:
        return _store(request).create_hypothesis(
            body.title, body.thesis, body.status, body.tags
        ).__dict__
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def confirm_t_research_hypothesis(body: TResearchConfirmIn, request: Request):
    try:
        ...
        raise HTTPException(status_code=503, detail="做T研究确认所需数据暂不可用") from exc
    if pool["pool_id"] != body.pool_id or pool["as_of"] != body.as_of.isoformat():
        raise HTTPException(status_code=409, detail="短线观察池已变化，请刷新后重新确认")
    try:
        hypothesis = build_t_research_hypothesis(pool)
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    return _store(request).create_or_get_hypothesis_by_tag(
        f"short_pool:{pool['pool_id']}", **hypothesis
    ).__dict__
...
def get_run_card(run_id: str, request: Request):
    try:
        ...
        raise HTTPException(status_code=404, detail="run card not found") from exc
