...
验证覆盖本轮 203 项聚焦后端回归、3 项规则类型切换前端回归、前端 TypeScript + Vite production build，以及自选分组、AI 草稿、异动监测、规则方向归一、未复权临时 OHLC 独立展示和交易路由的运行态 UI smoke。PR 复审后进一步锁死：取消/超时终态不得提前释放仍在运行的执行槽；残留槽位期间 `/pipeline/run` 明确返回 409 而不伪装成已调度；`chart_live` 未复权日 K 只通过独立 `provisional_daily` 返回。未进入本轮的多日分时、情绪周期/主线识别与因子挖掘仍保持 4.3 的候选状态。
