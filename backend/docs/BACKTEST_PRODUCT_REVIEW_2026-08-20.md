...
> **评审日期**：2026-08-20
> **评审方式**：代码事实盘点（后端 24+9 端点 / 前端 6 tab 全组件遍历，全部带文件行号证据）+ 竞品对标（2026-08-19 调研结论：QuantConnect / vectorbt / TradingView / Portfolio Visualizer / 聚宽 / 米筐）+ 真实数据端到端验证。**两轮独立 review（后端/前端各一）发现 1 个 P0 + 5 个 P1 + 若干 P2/P3**（参数透传丢失、复跑丢 entry_fill/exit_fill、前后端对比上限不一致、fill-reachability 方法错误、寻优固化丢参数与溯源、MC 终值重复减 1、分钟 guard 位置过晚、cancel 基准归一化不对称等），已全部修复并补回归测试，提交 b4605a9；修复后 tests/backtest+tests/api 1039 项全绿、前端 tsc/bun 全绿。教训：并行多 agent 大范围编辑后必须做端到端真实路径验证 + 独立 review，仅靠单测与类型检查漏掉了上述静默语义分叉。
> **视角**：专业产品经理 + 一线使用者。
...
> **交付状态（2026-08-20 当日晚间更新）**：一至四期已全部实施并提交（F1-F16 交付，F17 经实测论证暂缓——`notice_date` 0/25945 覆盖且财务数据仅 1 年）。提交：9164853/85c5a92（一期）、c19e8c6/c53e171（二期）、bdeeef2/d8d7606（三期）、7b023f8/9007395（四期）。全仓 pytest 2928 passed、前端 tsc/build/bun 全绿、浏览器 E2E 通过（分钟 VWAP 与日 K 开盘价差 -0.7%~+3.7%、组合合成 120 重叠日、实验列表/转监控/复跑弹窗/折叠持久化实测）。§4.2 清单各行即为交付物；§6 路线图成为实施记录。
