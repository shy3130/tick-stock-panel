...
| 任务重启续跑 V1（2026-08-20） | ✅ 已完成 | `job_store.py` / `job_recovery.py`：DurableJob 原子写 + lease；lifespan 启动扫描 running→interrupted，不自动开跑。策略/因子 SSE：cancelled 落盘后禁止重启；interrupted 重连整单重跑并推 `event: resumed`。寻优/网格：`POST .../resume` 跳过已完成场景；寻优窗口冻结、缺曲线 DSR/PBO 置空。证据：`tests/backtest/test_job_recovery.py`、`test_optimizer.py`、`test_parameter_grid.py`、`tests/api/test_backtest_optimizer_api.py`、`test_backtest_parameter_grid_api.py`、`test_run_store_api.py`；回测+API 1088 项全绿，前端 tsc 通过。 |
...
12. **任务重启续跑 V1 不是独立 worker**：策略/因子任务落盘 DurableJob（lease/heartbeat），启动把外来 lease 的 `running` 标为 `interrupted` 且不自动开跑；前端重连同一 query 时整单重跑（引擎无中途 checkpoint），先推 `event: resumed`。寻优/网格 `POST .../resume` 跳过已完成场景；寻优窗口必须用磁盘冻结值，禁止 `resolve_window`；缺本进程训练曲线时 DSR/PBO 置空并标 `resumed_partial_diagnostics`，禁止伪造。用户取消必须立即落盘 `cancelled`，store.save 不得把 cancelled 复活为 running。不是 Redis/RQ，计算仍在 API 进程内线程，重启窗口内任务中断。
