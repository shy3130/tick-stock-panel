...
2. **P2 chart_live — ✅ 已实现 (2026-08-24；复权隔离修正 2026-08-25)**：`TencentChartSource` 固定调用 `web.ifzq.gtimg.cn/appstock/app/minute/query`，仅当前 CN 交易日、单 A 股、并且 provider 成功返回目标日空行时才触发；分钟 API 直接返回增量行，日 K API 通过独立 `provisional_daily` 返回同源未复权 bar，绝不混入 `rows` 的复权历史，前端以“未复权 · 独立展示”OHLC 卡片呈现。catalog 异常仍返回 503，历史分钟/trans 不会触发；前端日 K 与分时均显示“外部源·盘中临时数据”，设置页提供独立 `chart_live` scope。全部 HTTP mock；`test_tencent_chart.py` 锁死量额单位、日期、交易时段、复权标记、熔断与本地优先门控。
