...
用法: python run_factor_search.py
"""
...
OUT = HERE.parent / "strategy_factor_search.json"
...
def load_sample():
    ...
    sample = rng.sample(all_syms, min(N_SYM, len(all_syms)))
    df = (
        lf.filter(
            (pl.col("date") >= D0)
            & (pl.col("date") <= D1)
...
def compute_features_safe(close, op, high, low, vol, turn):
    from factor_dsl import compute_features
