"""regime 条件化应用：硬切换（regime_switch）vs 引擎软减仓（regime_flat）对照。

在 B 方案（mom_trend 接入真实引擎 canonical replay）的基础上，检验「按市场状态
切换策略」是否真加值：
  - regime_switch : 新建 builtin 策略 regime_conditional —— 牛市跑 mom_trend，
                    熊市硬切换到 pullback_to_support，并在牛→熊翻转日强平 mom 仓位。
  - regime_flat   : mom_trend + 引擎原生 leader_index regime（bear_weight=0 + scale_existing）
                    —— 牛市 mom_trend、熊市直接空仓（软减仓的极端态）。
  - 基线沿用 B 方案：mom_trend（无 regime）/ mom_trend_regime（软减仓 bear=0.3）/
                    pullback_to_support（熊市基准）。

严格复用 P9 的 400 只 universe（seed 20260723）+ 4 折测试段切分，与 B 方案口径完全可比；
数据仅来自 data/kline_daily_enriched/**/*.parquet，回测复用真实引擎（费/滑点5bps/风控）。

用法: cd backend && TICKFLOW_BACKTEST_MODE=inprocess .venv/Scripts/python.exe -m research.regime.run_regime_conditional
"""
...
OUT = REGIME_ARTIFACTS_DIR / "strategy_regime_conditional.json"

# ── 与 P9 / B 方案完全一致的 universe / 切分 ──
N_SYM = 400
SEED = 20260723
...
LEADER_REGIME = {"type": "leader_index", "ma": 60, "mode": "soft"}

# 对照配置（dict，便于携带 params 与 regime_filter）
CONFIGS = [
    dict(key="mom_trend", sid="custom_factor", formula=MOM_TREND, regime=None,
         mp=10, ps="score_weight", params=None,
         note="基线：主因子 mom_trend，无 regime"),
    dict(key="mom_trend_regime", sid="custom_factor", formula=MOM_TREND,
         regime={**LEADER_REGIME, "bear_weight": 0.3, "scale_existing": True},
         mp=10, ps="score_weight", params=None,
         note="基线：主因子 + regime 软减仓(bear=0.3, scale_existing)"),
    dict(key="regime_flat", sid="custom_factor", formula=MOM_TREND,
         regime={**LEADER_REGIME, "bear_weight": 0.0, "scale_existing": True},
         mp=10, ps="score_weight", params=None,
         note="引擎原生软减仓极端态：bull→mom_trend / bear→空仓(bear_weight=0)"),
    dict(key="pullback_to_support", sid="pullback_to_support", formula=None, regime=None,
         mp=5, ps="score_weight", params=None,
         note="熊市基准策略（缩量回踩）"),
    dict(key="regime_switch", sid="regime_conditional", formula=None, regime=None,
         mp=10, ps="score_weight", params={"bear_strategy": "pullback"},
         note="硬条件化：bull→mom_trend / bear→pullback_to_support（牛→熊翻转日强平 mom）"),
]
...
def main():
    t0 = time.time()
    print("[regime-cond] regime 条件化 canonical replay 启动", flush=True)
    ...
    for fid, tr0, tr1, te0, te1 in folds:
        ...
        for cfg in CONFIGS:
            ...
            if err:
                ...
                continue
            print(f"  {cfg['key']:<18} ret={pct(m['total_return'])} Sharpe={m['sharpe']:+.2f} "
    ...
    out = {
        "config": {
            "description": "regime 条件化 canonical historical replay（硬切换 vs 软减仓）",
            ...
            "note": "与 B 方案同 universe/同 4 折测试段；两腿参数固定，regime 信号为"
                    "等权指数 MA60（1 日滞后）。区间已影响后续选择，只能视为历史 replay。",
        },
        "folds": fold_records,
        "aggregate": agg,
    }
