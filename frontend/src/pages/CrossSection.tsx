    ...
    <div className="workspace-page h-full min-h-0">
      ...
      <main className="workspace-content min-h-0 flex-1 overflow-auto">
        <div className="mx-auto flex max-w-7xl flex-col gap-3">
          <section className="panel border-accent/25 bg-accent/5">
            <div className="panel-body flex items-start gap-2 text-xs leading-relaxed text-secondary !py-2.5">
          ...
          <form onSubmit={handleSubmit} className={cn(CARD)}>
          ...
          <nav className="workspace-toolbar overflow-x-auto border-b border-border pb-2" aria-label="横截面分析区域">
            {TABS.map(({ id, label, icon: Icon }) => (
              <button
                ...
                onClick={() => setTab(id)}
                className={cn(
                  'btn-ghost h-8 shrink-0 text-xs',
                  tab === id ? 'bg-accent/15 text-accent' : '',
                )}
              >
  ...
  return (
    <section className="space-y-4">
      <section className={cn(CARD)}>
        <div className="panel-body flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
...
function SectionHeading({ icon: Icon, title, hint }: { icon: LucideIcon; title: string; hint?: string }) {
  return (
    <div className="panel-header flex-col items-start gap-1 sm:flex-row sm:items-center">
  ...
  return (
    <div className={cn(CARD)}>
      <div className="panel-body !py-2.5">
        ...
        <p className="metric-value mt-1 truncate text-sm">{value}</p>
      </div>
    </div>
  )
}
...
function DataBoundary({ notes }: { notes: string[] }) {
  return (
    <section className="panel bg-base/45">
      <div className="panel-body flex items-start gap-2 !py-2.5">
        <Database className="mt-0.5 h-4 w-4 shrink-0 text-muted" />
        <div>
          <p className="text-xs font-medium text-secondary">数据来源与边界（Provenance）</p>
          <p className="mt-0.5 text-xs leading-relaxed text-muted">来源：本地 DuckDB/enriched 横截面及其本地财务快照；无外部拉取、无写入。</p>
          {notes.length > 0 && (
...
function QueryError({ message, onRetry }: { message: string; onRetry: () => void }) {
  return <section className="panel border-danger/40 bg-danger/5 p-5 text-center" role="alert"><AlertCircle className="mx-auto h-5 w-5 text-danger" /><p className="mt-2 text-sm font-medium text-foreground">研究数据读取失败</p><p className="mt-1 break-words text-xs text-danger">{message}</p><button type="button" onClick={onRetry} className={cn(BTN_GHOST, 'mt-3')}><RefreshCw className="h-3.5 w-3.5" />重试</button></section>
}
