...
const MODULES: GuideModule[] = [
  ...
  },
  {
    ...
    icon: LineChart,
    tone: 'indigo',
    purpose: '验证信号历史表现、比较因子统计、评估约束下的组合权重，并用分段稳定性、严格 IS/OOS Walk-Forward、Bootstrap、置换检验、参数扰动与训练/留出策略寻优检查稳健性。',
    workflow: ['先定义假设、样本区间、基准与费用口径', '运行策略、因子、参数网格或策略寻优；网格/寻优场景可一键回填策略表单再验证', '在运行历史中搜索、收藏、打标签，选择 2~8 次结果对比指标、配置差异、交易变化与净值曲线', '查看专业诊断（滚动指标、月度热图、相对基准、持仓期 MAE/MFE）后下载自包含 HTML/JSON/CSV、复跑或打印报告'],
    data: '历史 enriched 数据；每次成功运行固化配置、数据快照、成本、指标口径、基准与随机种子，刷新或重启后仍可检索、比较与复跑。策略寻优实验独立落盘，不写入策略池。',
    freshness: '这是历史研究而非实时决策；股票池无法证明历史时点时保留幸存者偏差告警，跨区间或口径不同的比较会触发可比性提醒。寻优只按训练期打分，留出通过不等于全局最优。',
  },
  {
    id: 'research-flow',
    index: '05',
    title: '研究流程与信号验证',
    routes: ['研究工作台', '信号记分卡', '横截面分析'],
    to: '/research/overview',
    icon: FlaskConical,
    tone: 'cyan',
    purpose: '浏览公开因子、创建不可变研究 Run、登记假设与定时 recap，并做只读统计与信号验证。',
    workflow: ['从因子目录进入通用工作台并完成 preflight', '创建 Run 后查看结果、事件、序列与数据谱系', '假设/证据与大盘·自选·策略池 recap 仍走既有接口；分析计算对单标的日收益做风险、绩效、ADF 与 GARCH 检验'],
    data: '因子 Run 读取统一 catalog/preflight/runs 契约；假设与 recap 读取已入库的 enriched 日 K；分析计算只读 canonical 日 K，不产生任何交易建议。',
