...
const MODULES: GuideModule[] = [
  ...
  },
  {
    ...
    workflow: ['添加标的并分组或置顶', '按字段排序筛选', '从标的进入分析、监控或交易计划'],
  ...
  },
  {
    ...
    icon: LineChart,
    tone: 'indigo',
    purpose: '验证信号历史表现、比较因子统计、评估约束下的组合权重与稳健性。',
    workflow: ['先定义假设和样本区间', '运行回测或因子分析', '阅读 Run Card、风险与出场原因'],
    data: '历史 enriched 数据；回测依赖可选 vectorbt 环境，因子和策略都记录运行配置。',
    freshness: '这是历史研究，不是实时决策引擎；样本终点受当前本地日线最新日期限制。',
  },
  {
    id: 'analysis',
    index: '05',
    title: '标的深度分析',
    routes: ['个股分析', '财务分析', '概念分析', '行业分析'],
    to: '/stock-analysis',
    icon: FileSearch,
    tone: 'rose',
  ...
  },
  {
    ...
    icon: Bot,
    tone: 'indigo',
    purpose: '通过只读工具查询行情、策略、回测和研究资料，辅助解释和整理研究问题。',
    workflow: ['先提出可核对的问题', '检查工具调用与来源', '将结论作为研究输入而非执行指令'],
    data: 'AI 只能读取已授权的工具结果和受限附件；模型与 profile 由设置显式配置。',
    freshness: '答案不会绕过本地数据主链路；AI 不荐股、不生成订单、不自动下单。',
  },
  {
    id: 'trading',
    index: '09',
