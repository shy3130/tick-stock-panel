...
export const METRIC_TERM_LIST: MetricTerm[] = [
  ...
  {
    term: 'portfolio_combine',
    name: '净值组合回测 (Portfolio Combine)',
    definition: '把多个已固化策略回测 Run 的日频净值在共同交易日历上按权重事后加权合成，得到组合级净值与指标。',
    direction: '用于评估多策略权重配比与再平衡频率；贡献为正的成分在共同窗口内增厚了组合收益。',
    caveat: '独立回测净值的事后合成，非共享资金池撮合——不模拟策略间资金竞争、同时满仓冲突或保证金约束；再平衡假设无摩擦；各成分的成交可行性以其独立回测为准，重叠交易日不足时无法合成。',
  },
]
