...
> 当前仓库是纯后端量化研究 MVP，不包含也不接受 `frontend/`、React/Vite 页面或其他自定义 Web 前端。历史面板资料仅保存在 `docs/archive/`，不得据此恢复前端。交付入口是 FastAPI、CLI、研究脚本和离线报告。
...
- “代码已写完”“接口能调用”或“GitHub 没有红灯”都不代表完成。
...
- 研究：Python 脚本、walk-forward/OOS 流程和可审计 JSON/HTML 产物。
- 包管理和验证命令：后端与研究代码统一使用 `uv`/Python。
...
数据源或数据插件
  ...
  -> 策略 / 监控 / 回测 / 分析服务
  -> FastAPI API / SSE
  -> CLI / 研究脚本 / 离线报告 / 外部 API 客户端
```

改动前应沿这条链路检查上下游。禁止通过 API 或脚本层绕过标准化、仓库、业务服务而直接读取某个数据源或本地文件。
...
| `backend/research/` | 研究协议、因子搜索、OOS 与诊断脚本 | 不被生产代码反向依赖；产物写入 `artifacts/` |
| `scripts/` | 仓库级启动、校验与运维脚本 | 不承载策略或研究业务逻辑 |
| `docs/archive/` | 已下线面板资料 | 仅供追溯，不能作为当前实现依据 |
...
数据源已经插件化。任何通用功能都必须通过 provider 能力和标准化数据集访问数据，不能把 TickFlow SDK 调用硬编码到策略、监控、回测、API 或研究流程中。
...
- 参数修改后要检查：持久化内容、运行时实例、策略结果缓存、监控实例和 API 响应是否全部更新。
...
- 监控计算应复用标准化实时行情和统一规则，不为 API、语音、飞书、企业微信分别计算。
...
- 修改选股、评分或排序逻辑时，同时核对历史策略结果、回测交易记录和 API 输出是否使用同一候选集和排序方向。

## 6. 缓存、并发与性能

项目同时存在仓库缓存、enriched 历史缓存、实时聚合、筛选缓存、策略结果缓存、回测矩阵缓存、mtime 缓存和 SSE。修改读写路径时必须列出受影响的缓存层。

### 6.1 缓存一致性

- 写数据后同步检查：持久化文件、内存缓存、generation/version、SSE 事件和 API 可见状态。
...
- API 层保持薄，不在路由中复制 provider、策略或回测逻辑。
- CLI 与研究脚本复用正式数据契约，输出路径必须通过 `research.paths` 或既有路径模块解析。
- JSON 产物必须包含配置、seed、数据范围和失败原因；不能静默丢弃无效候选。
- 离线 HTML 只作为研究报告，不得引入独立应用前端或新的构建链。
- 加载、空数据、无权限和执行失败都要有明确、可测试的返回状态。
- 不用客户端补丁掩盖后端单位或契约错误；契约问题应在数据边界修正。
...
| 研究脚本或报告 | 定向测试 + 固定 seed 复现 + 产物 schema/路径检查 |
| API 与研究契约同时变化 | 后端定向测试 + 对应研究入口回归 |
...
作者自审和维护者复审均按以下顺序进行，避免只看接口表象或新增代码：
...
5. **跟踪状态变化**：从写入一路检查持久化、缓存、generation、SSE 和 API 可见状态。
